...
Notice that the argument that a Higher Order Function takes is always another function. To make it easy, you can save that function to a variable beforehand 
and then just pass in that variable. This would look like:

var myFunc = function(){};

function(myFunc);

xx Common Higher Order Functions
...
If inner function returns true (or something "truthy"), that item is included in the new array. If it returns false (or something "falsey"), it doesn't include 
it in the new array.
.reduce()
...
If inner function returns true, the .find() finishes and returns that current value in the original array. If it returns false, it continues to search through 
the array until it returns true. If it never returns true (no items match the condition), it returns undefined
.findIndex()
Same as .find() but instead of returning the item in the array, it returns the index where it found the matching item
.some()
Returns either true or false
If inner function returns true, the .some() immediately returns true (because at least one of the items matches your condition). If the inner function returns 
false for every item in the array, the .some() returns false (no items matches your condition)
.every()
Returns true or false
Opposite of .some(), in that if the inner function ever returns false for any item, .every() immediately returns false (because NOT every item matched the condition). 
If the inner function returns true for every item in the array, the .every() returns true

xx Functions: We are only going to be looking at the Methods. If you want to learn more or review your knowledge regarding setInterval,setTimeout, or clearInterval, 
you can do so by clicking on each of the following title:

setInterval (Links to an external site.)
setTimeout (Links to an external site.)
clearInterval (Links to an external site.)

xx Examples: Let us begin with one of the more popular Methods.
...
// => [2,3,4,5]
Let us compare this to a normal for loop:
...
// => [2,3,4,5]
They do virtually the exact same thing. One important thing to take note of is that rather than writing arr[i] to access each item in the array, we use item. 
The word item is not a saved word. Best practice is to name item the singular version of whatever your array holds, for example:
...
// => [56, 27]
So basically whatever you return inside the callback function is the condition that it will check for each item in the array. In the example above it went 
through each number in the numbers array and checked if it was greater than 25. If it was greater then it pushed it into the new array, if it was not, it did 
not include it in the new array.
...
// newArr => [56,27]
Comparing the two we see how much simpler and more concise the .filter is compared to the old way. Hopefully this demonstrates how useful Higher Order Functions 
can be!
...
Take note of the s at the end of numbers in the return statement. We are grabbing the index of the original array and we are not using the number parameter 
that we created in the callback function.
numbers[i] = number * 2 correct
number[i] = number * 2 incorrect
So what is the point of having forEach() if it doesn't save anything? Well it could be used for testing purposes if you wanted to see what happened without 
actually making any changes. Or it could be used to to run a function on each item. However, 9 times out of ten you will use .map over .forEach.
...
// => true
These are not all of the available Higher Order Functions, but these are the most common. I encourage you to familiarize yourself with these because 
they will be EXTREMELY useful throughout your coding career.
*/